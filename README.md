...
This is a GPU accelerated Raymarcher that implements binned BVH (Bounded Volume Hierarchy), instancing and stochastic path tracing to visualize black hole relativistic light bending in everyday scenes in the Unity editor with GGX material properties, supporting accurate fresnel reflections. When no black hole is present, the tracer defaults to a standard linear path tracer. Additionally, some support is given for atmosphereic scattering and some mild NEE, but since such occlusion computations are extremely expensive to perform, it IS NOT performant. Looking into ways to make curved space shadow maps, because currently all lighting is real time! Scenes upwards of 5 million triangles are supported (slowly, ~20 fps), with 60-90 FPS achieved in scenes with 1 million triangles with 5 reflections per ray. Refraction development is ongoing!
