...
/// <summary>
/// Bakes a 2D LUT indexed by (radius, sun_angle) into an RGBAFloat Texture2D.
///
/// Performance changes vs original:
///   1. MarchMaxSteps reduced from 500,000 to 50,000 (matches JS reference).
///   2. Early-exit for definitely-captured rays: if r is near the horizon and
///      the photon is moving inward, bail immediately instead of spinning.
///   3. Analytic bend-rate formula replaces bilinear LUT sampling in the inner
///      loop, eliminating index math + 4 array reads + 2 lerps per step.
///      BendLutData is retained for callers that need it, but the marcher no
///      longer uses it.
///   4. Indirect bracket scan step doubled (ex10 += 2) — halves march count
///      with negligible bracket precision loss.
///   5. FindCriticalAlpha early-exit: if the hi boundary already escapes,
///      return immediately.
/// </summary>
public static class DirectionalGeodesic2DLutSolver
{
    ...
    // Step size parameters — must match the GPU shader exactly.
    // The GPU computes:
    //   adaptiveStep = distFromHorizon * StepSize * StepRadialFrac
    //   stepLen      = sqrt(adaptiveStep^2 + StepMin^2)   (no cap)
    // where distFromHorizon = nearestPointOnSphere(..., rs) and StepMin is absolute.
    ...
    /// <summary>
    /// Standard march budget. 50,000 is sufficient for direct rays and
    /// for captured-ray detection (aided by the early-exit check).
    /// </summary>
    public static int MarchMaxSteps = 50_000;

    /// <summary>
    /// Extended march budget used for indirect rays, which must orbit the
    /// photon sphere and can require far more steps — especially when the
    /// observer is close to the horizon and alphaCrit is small.
    /// </summary>
        ...
        float rs = 1f)
    {
        ...
        // rs is explicit from the caller. marchRMax is the ray escape boundary,
        // which must be well beyond the furthest observer position so rays
        // launched from outer cells have room to escape.
        // Matches the JS: rMax = max(22*rs, r0 * 2.5).
    ...
    // -------------------------------------------------------------------------
    // March — analytic bend rate, no LUT sampling
    // -------------------------------------------------------------------------

    /// <summary>
    /// Integrates a null geodesic using adaptive RK2 and the exact analytic
    /// Schwarzschild bend-rate formula.  theta is stored raw (not wrapped) so
    /// the indirect solver can see values greater than pi.
    ///
    /// FIX 2: Early exit when the ray is definitely captured — saves the bulk
    /// of wasted iterations that caused the hour-long bake time.
    ///
    /// FIX 3: AnalyticBendRate replaces SampleBendRate, eliminating per-step
    /// memory indirection and interpolation error.
    /// </summary>
        ...
        int maxStepsOverride = 0)
    {
        ...
        // GPU-matched adaptive step size:
        //   adaptiveStep = (r - rs) * StepSize * StepRadialFrac
        //   stepLen      = sqrt(adaptiveStep^2 + StepMin^2)
        // StepMin is absolute (not rs-relative) matching the GPU's hardcoded 0.01.
        // StepMax is float.MaxValue (no cap) matching the GPU.
    ...
    /// <summary>
    /// LUT-based march — uses the bend LUT for per-step integration,
    /// matching the GPU shader exactly. This is the correct integrator
    /// to use when baking the Light LUT and for validation.
    /// </summary>
    public static bool March(
        float r0, float alpha0,
        float rs, float rMax,
    ...
    /// <summary>
    /// Same as March but also fills a list of (r, phi) polar points for
    /// visualisation. The list is cleared before use.
    /// maxPoints limits memory use — points are subsampled if the march
    /// produces more steps than maxPoints.
    /// </summary>
    ...
    /// <summary>
    /// Finds the largest emission angle (relative to outward radial) that still
    /// allows the photon to escape.
    /// FIX 5: Returns hi immediately if the hi test ray already escapes,
    /// saving BisectIterations marches in that edge case.
    /// </summary>
    ...
    // -------------------------------------------------------------------------
    // Legacy LUT sampling — kept so external callers don't break, but is no
    // longer called from the marcher.
    // -------------------------------------------------------------------------

    /// <summary>
    /// Bilinear sample of the bend LUT.
    /// No longer used internally; kept for external compatibility.
    /// </summary>
